...
- 메뉴별 구슬 수량(1~5개) 설정 가능
- 우승 기준 선택 가능
  - 마지막까지 남은 구슬 우승
  - n번째로 FINISH에 들어간 구슬 우승
...
- 실시간 랭킹 오버레이에서 전체 구슬 순서 확인 가능
- FINISH를 통과한 구슬은 체크 표시로 구분
...
### 히스토리 & 통계
- 최근 30개 뽑기 기록 시간순 표시
- 기록 시간대에 따라 아침 / 점심 / 저녁 / 야식 태그 표시
- TOP 5 메뉴 바 차트
- 선호 카테고리 1위 표시
- 통계 패널 안 `요약 / 날짜별 기록` 전환 지원
- 최근 7일 안 먹은 메뉴 빠른 조회
- 기록 전체 삭제
...
- 최근 검색어 + 현재 선택 메뉴 기반 검색어 추천 자동완성
- 검색 결과 정렬 지원
  - 가까운 순
  - 이름순
  - 저장한 가게 우선
